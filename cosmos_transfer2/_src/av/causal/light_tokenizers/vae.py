...
class Resample(nn.Module):
    ...
    def _upsample_time(self, x, b, c, t, h, w):
        """Reshape time_conv output [b, 2c, t, h, w] into interleaved [b, c, 2t, h, w]."""
        x = x.reshape(b, 2, c, t, h, w)
        x = torch.stack((x[:, 0, :, :, :, :], x[:, 1, :, :, :, :]), 3)
        return x.reshape(b, c, t * 2, h, w)

    def forward(self, x, feat_cache=None, feat_idx=[0], parallel=False):
        b, c, t, h, w = x.size()
        if self.mode == "upsample3d":
            if parallel:
                # Parallel mode: process all temporal frames in one pass
                idx = feat_idx[0] if feat_cache is not None else None

                if feat_cache is None:
                    # No cache tracking — one-shot decode of full video
                    first_frame = x[:, :, :1, :, :]
                    rest = x[:, :, 1:, :, :]
                    if rest.shape[2] > 0:
                        rest_up = self.time_conv(rest)
                        rest_up = self._upsample_time(rest_up, b, c, rest.shape[2], h, w)
                        x = torch.cat([first_frame, rest_up], dim=2)

                elif feat_cache[idx] is None:
                    # Very first chunk of video — frame 0 has no temporal upsample
                    first_frame = x[:, :, :1, :, :]
                    rest = x[:, :, 1:, :, :]
                    if rest.shape[2] > 0:
                        rest_up = self.time_conv(rest)
                        rest_up = self._upsample_time(rest_up, b, c, rest.shape[2], h, w)
                        x = torch.cat([first_frame, rest_up], dim=2)
                        # Cache from rest (frames that went through time_conv), not full x
                        cache_x = rest[:, :, -CACHE_T:, :, :].clone()
                        ...
                        # Only frame 0 — no time_conv applied, match sequential sentinel
                        feat_cache[idx] = "Rep"
                    feat_idx[0] += 1

                elif feat_cache[idx] == "Rep":
                    # Previous chunk was just frame 0; all frames get zero-padded time_conv
                    ...
                    x_up = self.time_conv(x)  # full causal zero-padding, no cache
                    x = self._upsample_time(x_up, b, c, t, h, w)
                    feat_cache[idx] = cache_x
                    feat_idx[0] += 1

                else:
                    # Normal subsequent chunk — all frames get time_conv with cache
                    cache_x = x[:, :, -CACHE_T:, :, :].clone()
                    if cache_x.shape[2] < CACHE_T:
                        cache_x = torch.cat(
                    ...
                    x_up = self.time_conv(x, feat_cache[idx])
                    x = self._upsample_time(x_up, b, c, t, h, w)
                    feat_cache[idx] = cache_x
                    feat_idx[0] += 1
...
class Decoder3d(nn.Module):
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0], parallel=False):
        ...
        for layer in self.upsamples:
            if parallel and isinstance(layer, Resample):
                x = layer(x, feat_cache=feat_cache, feat_idx=feat_idx, parallel=True)
            elif feat_cache is not None:
                x = layer(x, feat_cache, feat_idx)
            else:
                x = layer(x)
...
class WanVAE_(nn.Module):
    def __init__(
        ...
        if parallel_decode:
            logger.info("Using parallel decode")
    ...
    def forward(self, x, scale, cache: Optional[WANVAECache] = None):
        mu, log_var = self.encode(x, scale, cache=cache)
        z = self.reparameterize(mu, log_var)
        x_recon = self.decode(z, scale, cache=cache)
    ...
    def tiled_encode(self, x, scale, cache: Optional[WANVAECache] = None):
        ...
        for i in range(0, height, self.tile_sample_stride_height):
            ...
            for j in range(0, width, self.tile_sample_stride_width):
                ...
                for k in range(frame_range):
                    ...
                    tile = self.encoder(
                        tile,
                        feat_cache=self._enc_feat_map if cache is None else cache.enc_feat_map,
                        feat_idx=self._enc_conv_idx if cache is None else cache.enc_conv_idx,
                    )
    ...
    def tiled_decode(self, z, scale, cache: Optional[WANVAECache] = None):
        if isinstance(scale[0], torch.Tensor):
        ...
        for i in range(0, height, tile_latent_stride_height):
            ...
            for j in range(0, width, tile_latent_stride_width):
                ...
                if self.parallel_decode:
                    # Parallel mode: decode all temporal frames at once
                    tile = z[:, :, :, i : i + tile_latent_min_height, j : j + tile_latent_min_width]
                    tile = self.conv2(tile)
                    if cache is None:
                        self._conv_idx = [0]
                        decoded = self.decoder(tile, feat_cache=self._feat_map, feat_idx=self._conv_idx, parallel=True)
                    else:
                        cache.dec_conv_idx = [0]
                        decoded = self.decoder(
                            tile, feat_cache=cache.dec_feat_map, feat_idx=cache.dec_conv_idx, parallel=True
                        )
                    row.append(decoded)
                else:
                    time = []
                    for k in range(num_frames):
                        ...
                        decoded = self.decoder(
                            tile,
                            feat_cache=self._feat_map if cache is None else cache.dec_feat_map,
                            feat_idx=self._conv_idx if cache is None else cache.dec_conv_idx,
                        )
                        time.append(decoded)
                    row.append(torch.cat(time, dim=2))
            rows.append(row)
    ...
    def _i0_encode(self, x, cache: Optional[WANVAECache] = None):
        """
    ...
    def encode_single_chunk(self, x, scale, cache: Optional[WANVAECache] = None):
        # cache
        t = x.shape[2]
        ...
        if cache is None:
            ...
            cache.enc_conv_idx = [0]
        if is_first_chunk:
            out = self._i0_encode(x, cache=cache)
        else:
            out = self.encoder(
                x,
                feat_cache=self._enc_feat_map if cache is None else cache.enc_feat_map,
    ...
    def encode(self, x, scale, return_mu=False, cache: Optional[WANVAECache] = None):
        if x.shape[2] in [1, 4]:
            # Cache is handled outside of this class, and input is passed in chunk by chunk.
            return self.encode_single_chunk(x, scale, cache)
        ...
        # -- Added in Wan2pt1 WanVAE_ implementation
        TEMPORAL_WINDOW = 4
        if (t - 1) % TEMPORAL_WINDOW:
            if cache is None:
                self._enc_conv_idx = [0]
            else:
                cache.enc_conv_idx = [0]
            out_ = self.encoder(
                x[:, :, 1 + TEMPORAL_WINDOW * (iter_ - 1) :, :, :],
                feat_cache=self._enc_feat_map if cache is None else cache.enc_feat_map,
                feat_idx=self._enc_conv_idx if cache is None else cache.enc_conv_idx,
            )
            out = torch.cat([out, out_], 2)
    ...
    def decode(self, z, scale, cache: Optional[WANVAECache] = None):
        self.clear_cache()
        ...
        if self.parallel_decode:
            # Parallel mode: decode all frames at once
            if cache is None:
                self._conv_idx = [0]
                out = self.decoder(x, feat_cache=self._feat_map, feat_idx=self._conv_idx, parallel=True)
            else:
                cache.dec_conv_idx = [0]
                out = self.decoder(x, feat_cache=cache.dec_feat_map, feat_idx=cache.dec_conv_idx, parallel=True)
        else:
    ...
    def decode_stream(self, z, scale, cache: Optional[WANVAECache] = None):
        self.clear_cache()
    ...
    def cached_decode(self, z, scale, cache: Optional[WANVAECache] = None):
        # z: [b,c,t,h,w]
        if isinstance(scale[0], torch.Tensor):
            z = z / scale[1].view(1, self.z_dim, 1, 1, 1) + scale[0].view(1, self.z_dim, 1, 1, 1)
        else:
    ...
    def sample(self, imgs, deterministic=False, scale=[0, 1], cache: Optional[WANVAECache] = None):
        mu, log_var = self.encode(imgs, scale, return_mu=True, cache=cache)
        if deterministic:
            return mu
    ...
    def prepare_cache(self) -> WANVAECache:
        enc_conv_num = count_conv3d(self.encoder)
        dec_conv_num = count_conv3d(self.decoder)
        return WANVAECache(
            enc_conv_idx=[0],
            enc_feat_map=[None] * enc_conv_num,
            dec_conv_idx=[0],
            dec_feat_map=[None] * dec_conv_num,
        )

    def encode_video(self, x, scale=[0, 1], cache: Optional[WANVAECache] = None):
        assert x.ndim == 5  # NTCHW
        assert x.shape[2] % 3 == 0
        x = x.transpose(1, 2)
        y = x.mul(2).sub_(1)
        y, mu, log_var = self.sample(y, scale=scale, cache=cache)
        return y.transpose(1, 2).to(x), mu, log_var

    def decode_video(self, x, scale=[0, 1], cache: Optional[WANVAECache] = None):
        assert x.ndim == 5  # NTCHW
        assert x.shape[2] % self.z_dim == 0
        x = x.transpose(1, 2)
        # B, C, T, H, W
        y = x
        y = self.decode(y, scale, cache=cache).clamp_(-1, 1)
...
class WanVAE:
    ...
    def to_cuda(self, device: str | torch.device = "cuda"):
    ...
    def encode_dist(self, video, world_size, cur_rank, split_dim, cache: Optional[WANVAECache] = None):
        spatial_ratio = 8
        ...
        if self.use_tiling:
            encoded_chunk = self.model.tiled_encode(video_chunk, self.scale, cache=cache)
        else:
            encoded_chunk = self.model.encode(video_chunk, self.scale, cache=cache)
        ...
        dist.all_gather(full_encoded, encoded_chunk, group=self.process_group)
    ...
    def encode_dist_2d(
        self, video, world_size_h, world_size_w, cur_rank_h, cur_rank_w, cache: Optional[WANVAECache] = None
    ):
        spatial_ratio = 8
    ...
    def encode(self, video, world_size_h=None, world_size_w=None, cache: Optional[WANVAECache] = None):
        """
        ...
        if self.parallel:
            ...
            cur_rank = dist.get_rank(self.process_group)
            ...
            if self.use_2d_split:
                ...
                out = self.encode_dist_2d(video, world_size_h, world_size_w, cur_rank_h, cur_rank_w, cache=cache)
            else:
                # Original 1D splitting logic
                if width % world_size == 0:
                    out = self.encode_dist(video, world_size, cur_rank, split_dim=4, cache=cache)
                elif height % world_size == 0:
                    out = self.encode_dist(video, world_size, cur_rank, split_dim=3, cache=cache)
                else:
                    logger.info("Fall back to naive encode mode")
                    if self.use_tiling:
                        out = self.model.tiled_encode(video, self.scale, cache=cache)
                    else:
                        out = self.model.encode(video, self.scale, cache=cache)
        else:
            if self.use_tiling:
                out = self.model.tiled_encode(video, self.scale, cache=cache)
            else:
    ...
    def decode_dist(self, zs, world_size, cur_rank, split_dim, cache: Optional[WANVAECache] = None):
        total_len = zs.shape[split_dim]
        chunk_len = total_len // world_size
        padding_size = 1

        if cur_rank == 0:
            start = 0
            length = chunk_len + 2 * padding_size
        elif cur_rank == world_size - 1:
            start = total_len - (chunk_len + 2 * padding_size)
            length = chunk_len + 2 * padding_size
        else:
            start = cur_rank * chunk_len - padding_size
            length = chunk_len + 2 * padding_size
        zs = torch.narrow(zs, split_dim, start, length).contiguous()

        decode_func = self.model.tiled_decode if self.use_tiling else self.model.decode
        images = decode_func(zs, self.scale, cache=cache).clamp_(-1, 1)

        pixel_chunk = chunk_len * 8
        if cur_rank == 0:
            images = torch.narrow(images, split_dim, 0, pixel_chunk).contiguous()
        elif cur_rank == world_size - 1:
            images = torch.narrow(images, split_dim, images.shape[split_dim] - pixel_chunk, pixel_chunk).contiguous()
        else:
            images = torch.narrow(images, split_dim, 8 * padding_size, pixel_chunk).contiguous()

        full_images = [torch.empty_like(images) for _ in range(world_size)]
        dist.all_gather(full_images, images, group=self.process_group)

        self.device_synchronize()

        images = torch.cat(full_images, dim=split_dim)

        return images

    def decode_dist_2d(
        self, zs, world_size_h, world_size_w, cur_rank_h, cur_rank_w, cache: Optional[WANVAECache] = None
    ):
        total_h = zs.shape[-2]
        total_w = zs.shape[-1]
        ...
        # Extract the latent chunk for this process (slice last two dims: H and W)
        zs_chunk = zs[..., h_start:h_end, w_start:w_end].contiguous()

        # Decode the chunk
        decode_func = self.model.tiled_decode if self.use_tiling else self.model.decode
        images_chunk = decode_func(zs_chunk, self.scale, cache=cache).clamp_(-1, 1)
        ...
        dist.all_gather(full_images, images_chunk, group=self.process_group)
    ...
    def decode_dist_2d_stream(
        ...
        for image in self.model.decode_stream(zs_chunk, self.scale, cache=cache):
    ...
    def decode(self, zs, cache: Optional[WANVAECache] = None):
        if self.cpu_offload:
            self.to_cuda()
        ...
        if self.parallel:
            world_size = dist.get_world_size(self.process_group)
            cur_rank = dist.get_rank(self.process_group)
            latent_height, latent_width = zs.shape[-2], zs.shape[-1]
            if self.use_2d_split:
                world_size_h, world_size_w = self._calculate_2d_grid(latent_height, latent_width, world_size)
                cur_rank_h = cur_rank // world_size_w
                cur_rank_w = cur_rank % world_size_w
                images = self.decode_dist_2d(zs, world_size_h, world_size_w, cur_rank_h, cur_rank_w, cache=cache)
            else:
                # Original 1D splitting logic
                if latent_width % world_size == 0:
                    images = self.decode_dist(zs, world_size, cur_rank, split_dim=zs.ndim - 1, cache=cache)
                elif latent_height % world_size == 0:
                    images = self.decode_dist(zs, world_size, cur_rank, split_dim=zs.ndim - 2, cache=cache)
                else:
                    logger.info("Fall back to naive decode mode")
                    images = self.model.decode(zs, self.scale, cache=cache).clamp_(-1, 1)
        else:
            decode_func = self.model.tiled_decode if self.use_tiling else self.model.decode
            images = decode_func(zs, self.scale, cache=cache).clamp_(-1, 1)
    ...
    def decode_stream(self, zs, cache: Optional[WANVAECache] = None):
        if self.cpu_offload:
            self.to_cuda()
        ...
        if self.parallel:
            ...
            for images in self.decode_dist_2d_stream(
                zs, world_size_h, world_size_w, cur_rank_h, cur_rank_w, cache=cache
            ):
                yield images
        else:
            for image in self.model.decode_stream(zs, self.scale, cache=cache):
    ...
    def encode_video(self, vid, cache: Optional[WANVAECache] = None):
        return self.model.encode_video(vid, cache=cache)

    def decode_video(self, vid_enc, cache: Optional[WANVAECache] = None):
        return self.model.decode_video(vid_enc, cache=cache)

    def device_synchronize(
        self,
    ):
        # if type(self.device) == torch.device:
        #     torch.cuda.synchronize()
        # else:
        #     torch_device_module = getattr(torch, self.device)
