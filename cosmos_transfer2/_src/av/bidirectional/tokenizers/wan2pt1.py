...
from cosmos_transfer2._src.av.bidirectional.utils.tokenizer_benchmarking import BenchmarkTimes
...
class WanVAE_(nn.Module):
    def __init__(
        ...
        dropout=0.0,
        temporal_window=4,
        reset_cache_in_encoding=True,
        reset_cache_in_decoding=True,
    ):
        super().__init__()
        self.dim = dim
        self.z_dim = z_dim
        ...
        self.reset_cache_in_encoding = reset_cache_in_encoding
        self.reset_cache_in_decoding = reset_cache_in_decoding
    ...
    def encode(self, x, scale, cache: Optional[WANVAECache] = None):
        if not self.reset_cache_in_encoding and x.shape[2] in [1, self.temporal_window]:
    ...
    def decode(self, z, scale, cache: Optional[WANVAECache] = None):
        if self.reset_cache_in_decoding:
            self.clear_cache()
        # z: [b,c,t,h,w]
        if isinstance(scale[0], torch.Tensor):
...
def _video_vae(
    ...
    **kwargs,
    ...
    return (
        ...
        torch.zeros(1, 1, max_video_length, 1, 1, device=device),
        torch.ones(1, 1, max_video_length, 1, 1, device=device),
    )
...
class WanVAE:
    def __init__(
        ...
        s3_credential_path: str = "credentials/s3_training.secret",
        load_mean_std=False,
        mean_std_path: str = None,
        dtype=torch.float,
        device="cuda",
        is_amp=True,
        benchmark: bool = False,
        temporal_window: int = 4,
        reset_cache_in_encoding: bool = True,
        reset_cache_in_decoding: bool = True,
        max_video_length: int = 500,
    ):
        # default weight path when not provided
        if vae_pth is None:
            for default_path in [
                "/lustre/fsw/portfolios/nvr/users/zianw/cache/imaginaire/bucket/cosmos_diffusion_v2/pretrain_weights/tokenizer/wan2pt1/Wan2.1_VAE.pth",
                "/lustre/fsw/portfolios/av/users/zianw/cache/imaginaire/bucket/cosmos_diffusion_v2/pretrain_weights/tokenizer/wan2pt1/Wan2.1_VAE.pth",
                "/nfs/av/av_alpamayo_cosmos/users/zianw/cache/imaginaire/bucket/cosmos_diffusion_v2/pretrain_weights/tokenizer/wan2pt1/Wan2.1_VAE.pth",
                "/home/ruilongl/workspace/storage/tokenizer/wan2pt1/Wan2.1_VAE.pth",
            ]:
                if os.path.exists(default_path):
                    vae_pth = default_path

            assert vae_pth is not None

        if mean_std_path is None:
            for default_path in [
                "/lustre/fsw/portfolios/nvr/users/zianw/cache/imaginaire/bucket/cosmos_diffusion_v2/pretrain_weights/tokenizer/wan2pt1/mean_std.pt",
                "/lustre/fsw/portfolios/av/users/zianw/cache/imaginaire/bucket/cosmos_diffusion_v2/pretrain_weights/tokenizer/wan2pt1/mean_std.pt",
                "/nfs/av/av_alpamayo_cosmos/users/zianw/cache/imaginaire/bucket/cosmos_diffusion_v2/pretrain_weights/tokenizer/wan2pt1/mean_std.pt",
            ]:
                if os.path.exists(default_path):
                    mean_std_path = default_path

            if load_mean_std:
                assert mean_std_path is not None
        ...
        self.model, self.img_mean, self.img_std, self.video_mean, self.video_std = _video_vae(
            ...
            reset_cache_in_encoding=reset_cache_in_encoding,
            reset_cache_in_decoding=reset_cache_in_decoding,
    ...
    @torch.no_grad()
    def encode(self, videos, cache: Optional[WANVAECache] = None):
        """
        ...
        with self.context:
            ...
            latent = self.model.encode(videos, self.scale, cache)
            if self.benchmark:
                torch.cuda.synchronize()
    ...
    def decode(self, zs, cache: Optional[WANVAECache] = None):
        ...
        with self.context:
            ...
            if self.benchmark:
                torch.cuda.synchronize()
                model_time = time.perf_counter()
            video_recon = self.model.decode(zs, self.scale, cache)
            if self.benchmark:
                torch.cuda.synchronize()
                benchmark_times.model_invocation = time.perf_counter() - model_time
...
class Wan2pt1VAEInterface(VideoTokenizerInterface):
    def __init__(self, chunk_duration: int = 81, load_mean_std=False, max_video_length: int = 500, **kwargs):
        self.device = kwargs.get("device", "cuda")
        self.model = WanVAE(
            dtype=torch.bfloat16,
            is_amp=False,
            load_mean_std=load_mean_std,
            vae_pth=kwargs.get("vae_pth", None),
            ...
            reset_cache_in_encoding=kwargs.get("reset_cache_in_encoding", True),
            reset_cache_in_decoding=kwargs.get("reset_cache_in_decoding", True),
            device=self.device,
            max_video_length=max_video_length,
        )
    ...
    def encode(self, state: torch.Tensor, cache: Optional[WANVAECache] = None) -> torch.Tensor:
        latents = self.model.encode(state, cache=cache)
